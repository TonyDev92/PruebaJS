...
const noVocalName$$ = document.querySelector('#nameWithoutVocals')

function calculateIntervalDays(date) {
    const newDate = handleInputDate(inputDate$$.value);
    let msecPerDay = 1000 * 60 * 60 * 24;
    const defaultDate = new Date('March,15,2023').getTime();
    const daysDate = newDate == defaultDate ? 0 : newDate - defaultDate; 
    return Math.floor(daysDate / msecPerDay);
}

function removeVocalsAndSpaces() {
    const newName = handleInputName();
    const noSpace = newName.replace(/ /g, "");
    return noSpace.replace(/[aeiou]/ig,'');
}
...
name$$.addEventListener('blur', () => noVocalName$$.value = removeVocalsAndSpaces());
inputDate$$.addEventListener('blur', () => days$$.value = calculateIntervalDays(inputDate$$.value));
